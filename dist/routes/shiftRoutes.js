...
router.get('/', async (req, res) => {
    try {
        const shifts = await Shift_1.default.find({ isActive: true }).sort({ name: 1 });
        res.json(shifts);
    }
    catch (error) {
...
router.put('/:id', async (req, res) => {
    try {
        const updatedShift = await Shift_1.default.findByIdAndUpdate(req.params.id, req.body, { new: true, runValidators: true });
        if (!updatedShift) {
...
router.delete('/:id', async (req, res) => {
    try {
        const shift = await Shift_1.default.findByIdAndUpdate(req.params.id, { isActive: false }, { new: true });
        if (!shift) {
            return res.status(404).json({ error: 'Shift not found' });
        }
...
router.post('/initialize-defaults', async (req, res) => {
    try {
        ...
        const existingShifts = await Shift_1.default.find({});
        if (existingShifts.length === 0) {
            const createdShifts = await Shift_1.default.insertMany(defaultShifts);
